...
const PTEX = (f: string) => `${BASE}textures/planets/${f}`;

interface PlanetDef {
  id: string;
  name: string;
  color: string;
  /** Texture file (Solar System Scope, CC BY 4.0). */
  tex: string;
...
const PLANETS: PlanetDef[] = [
  ...
  { id: 'planet-uranus', name: 'Uranus', color: '#8fd2d8', tex: PTEX('2k_uranus.jpg'), size: 1.9, au: 19.2, periodY: 84 },
  { id: 'planet-neptune', name: 'Neptune', color: '#4c6fdd', tex: PTEX('2k_neptune.jpg'), size: 1.85, au: 30.1, periodY: 165 },
];
...
function Planet({ def, onSelect }: { def: PlanetDef; onSelect: (id: string) => void }) {
  ...
  const map = useLoader(THREE.TextureLoader, def.tex);
  useEffect(() => {
...
function Sun() {
  ...
  const sunMap = useLoader(THREE.TextureLoader, PTEX('2k_sun.jpg'));
  useEffect(() => {
